...
// h / l, which the old handler stole for list-cursor movement (typing "kaveh.org" lost
...
func TestExploreJumpTypesEveryPrintableRune(t *testing.T) {
	...
	for _, r := range "kaveh.org" {
		v.handleKey(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{r}})
	}
	if v.jump.query != "kaveh.org" {
		t.Fatalf("typing kaveh.org into JUMP produced %q - printable runes are being stolen", v.jump.query)
	}
	// And the frame paints exactly what was typed (the input is honest on screen too).
	if out := a.View(); !strings.Contains(out, "kaveh.org") {
