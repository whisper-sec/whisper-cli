...
// path (the release build) stamps the real the release version so a served
// binary's `whisper --version` matches the release tag exactly . This literal
...
type globalFlags struct {
	...
	noColor    bool
	themeName  string
	// guided-flow selectors: drive bare `whisper` headlessly.
