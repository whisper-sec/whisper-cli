...
func graphStub(t *testing.T) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.Contains(q, "db.schema"):
			schema := `{"nodes":[{"label":"IPV4","count":128,"virtual":false,"properties":[{"name":"name","type":"String"}]},{"label":"HOSTNAME","count":512,"virtual":false,"properties":[]}],"relationships":[{"type":"RESOLVES_TO","source":["HOSTNAME"],"target":["IPV4","IPV6"],"count":1,"virtual":false},{"type":"LOCATED_IN","source":["IPV4"],"target":["CITY"],"count":1,"virtual":false}]}`
			enc, _ := json.Marshal(schema)
			_, _ = w.Write([]byte(`{"columns":["schema"],"rows":[{"schema":` + string(enc) + `}]}`))
		case strings.Contains(q, "db.labels"):
			_, _ = w.Write([]byte(`{"columns":["label","count"],"rows":[{"label":"HOSTNAME","count":512},{"label":"IPV4","count":128}]}`))
