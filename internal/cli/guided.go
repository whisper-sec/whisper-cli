...
// stack trace. Every interactive branch has a headless equivalent driven by flags
...
func runGuided(opts guidedOptions, gio guidedIO) error {
	...
	// entirely: the caller already told us what to do.
	if opts.create {
...
func guidedMany(c *client.Client, opts guidedOptions, gio guidedIO, choices []agentChoice) error {
	...
	for {
		...
		if rerr != nil {
			...
			// documented headless error (the decision-table path), not a silent default.
			return usageErr("you have multiple agents - pass --agent <id|name> to choose one")
		}
